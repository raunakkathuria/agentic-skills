...
description: >-
  Apply Raunak's engineering leadership principles to engineering projects,
  sprint planning, architecture decisions, code reviews, incident response,
  data analysis, and team discussions. Use when planning, building, reviewing,
  operating, or explaining engineering work.
license: MIT
metadata:
...
These are Raunak's distilled principles for how to approach every engineering
project: sprint planning, PR reviews, architecture decisions, funnel analysis,
incident retrospectives, data analysis, feature scoping, CI/CD setup, test
strategy, and project kickoff. Apply them proactively — don't wait to be asked.
...
Never analyse before validating the data source. Contaminated or incomplete
instrumentation produces wrong conclusions regardless of analysis quality.
...
High-level metrics hide the real problem. Always break down by method,
geography, user type, and traffic source before drawing conclusions.
...
Rank problems by failure rate or revenue impact. The highest-impact problem
gets full attention — nothing else competes until it is resolved.
...
Short feedback loops beat big-bang delivery. Monitoring is part of
Definition of Done, not a post-launch task.
...
Ten people on one problem beat ten people on ten problems. The War Room
model works because there is no coordination overhead.

- One priority at a time during sprints — protect the focus
- Co-locate or keep a permanent Zoom open — remove the lag between question and answer
...
Complexity is a liability. Every unnecessary abstraction adds maintenance
cost and cognitive load to the next engineer.
...
Build for today's requirements, not tomorrow's hypotheticals. Premature
generalisation adds complexity with zero benefit.
...
Write the test first. The code is just making the test pass. TDD is a
design practice, not a coverage metric.
...
Broken code never reaches the next environment. Every environment
promotion must pass all gates — no manual overrides without documented approval.
...
Every system has a person (or team) accountable for it. No orphaned services,
no "someone else will fix it."
...
Define the success metric before building. Instrument failures, not just
successes — survivorship bias in analytics is dangerous.

- Structured logs with consistent schema (machine-parseable)
- Alert on symptoms: conversion drop in 5 min > CPU at 80%
- *Example:* PostHog claimed 100% deposit completion — actual data showed 35%
  fiat failure rate because only success events were tracked
...
Institutional knowledge must be explicit. Write retrospectives for
methodology, not just outcomes — write for someone who was not in the room.
...
The question is not "how can AI help here?" but "should a human be doing
this at all?" Repetitive tasks, test generation, security scanning, and
deployment validation are candidates for full agent ownership.
...
When writing about engineering leadership:

- Use real (anonymised) examples from engineering work
- Challenge conventional wisdom — find the counterintuitive truth
- Acknowledge trade-offs honestly — no silver bullets
- Make it practical — the reader should be able to apply it Monday morning
